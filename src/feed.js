...
export async function buildFeed(registry, tally, { includeAll = false, sort = 'plays' } = {}) {
  const [d7, d30, all, starts] = await Promise.all([tally.sites(7), tally.sites(30), tally.sites(3650), tally.countByName('start', 7)]);
  const by = (rows) => Object.fromEntries(rows.map((r) => [r.site, r]));
  const s7 = by(d7), s30 = by(d30), sAll = by(all), st = by(starts);
  const games = registry.all()
    .filter((r) => !r.manifest.hidden)
    .filter((r) => includeAll || r.status === 'live')
    .map((r) => {
      const m = r.manifest;
      const plays = { d7: s7[m.site]?.visitors ?? 0, d30: s30[m.site]?.visitors ?? 0, all: sAll[m.site]?.visitors ?? 0 };
      return {
  ...
  const cmp = sort === 'started'
    ...
    : (a, b) => b.plays.d7 - a.plays.d7 || b.plays.d30 - a.plays.d30 || b.plays.all - a.plays.all || (b.started || '').localeCompare(a.started || '') || a.slug.localeCompare(b.slug);
  games.sort(cmp);
...
export function cartsShape(feed) {
  return feed.games.map((g) => ({ s: g.slug, n: g.name, u: g.url, a: g.shell, plate: g.plate, rank: g.rank, plays: g.plays.d7, family: g.family, variant: g.variant || undefined }));
}
