...
export async function buildFeed(registry, tally, { includeAll = false, sort = 'plays', cat = '' } = {}) {
  ...
  const s7 = by(d7), s30 = by(d30), sAll = by(all), st = by(runs30), g7 = by(e7), g30 = by(e30);
  let games = registry.all()
  ...
  const cmp = sort === 'started'
    ...
    : (a, b) => b.plays.month - a.plays.month || b.plays.engaged30 - a.plays.engaged30 || b.plays.engaged7 - a.plays.engaged7 || b.plays.d30 - a.plays.d30 || b.plays.d7 - a.plays.d7 || b.plays.all - a.plays.all || (b.started || '').localeCompare(a.started || '') || a.slug.localeCompare(b.slug);
  games.sort(cmp);
  games.forEach((g, i) => { g.rank = i + 1; });
  const categories = {};
  for (const id of CATEGORY_IDS) { const n = games.filter((g) => g.category === id).length; if (n) categories[id] = n; }
  if (cat) games = games.filter((g) => g.category === cat);
  const families = {};
  for (const g of games) (families[g.family] ??= []).push(g.slug);
  return { generatedAt: Date.now(), count: games.length, sort, categories, games, families };
}

/** The four-field shape hundred-carts already reads, plus plate. */
export function cartsShape(feed) {
  return feed.games.map((g) => ({ s: g.slug, n: g.name, u: g.url, a: g.shell, plate: g.plate, rank: g.rank, plays: g.plays.month, playsBy: g.plays.by, visitors: g.plays.d30, category: g.category, family: g.family, variant: g.variant || undefined }));
}
