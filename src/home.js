...
export function homePage(feed, { publicUrl = '' } = {}) {
  ...
  const rows = feed.games.map((g) => `<li><a href="${esc(g.url)}"><img src="${esc(g.plate)}" alt="" loading="lazy"><b>${esc(g.name)}</b></a><span>${g.plays.d7} this week${g.variant ? ' · ' + esc(g.variant) : ''}${g.designers.length ? ' · ' + esc(g.designers.join(', ')) : ''}</span></li>`).join('\n');
