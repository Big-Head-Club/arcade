...
export function homePage(feed, { publicUrl = '' } = {}) {
  ...
  const rows = feed.games.map((g) => `<li><a href="${esc(g.url)}"><img src="${esc(g.plate)}" alt="" loading="lazy"><b>${esc(g.name)}</b></a><span>${g.plays.week} ${g.plays.by === 'runs' ? 'runs' : 'played'} this week${g.platform && g.platform !== 'unknown' ? ' · ' + ({ mobile: '📱 mobile', desktop: '🖥 desktop', both: '📱🖥' })[g.platform] : ''}${g.variant ? ' · ' + esc(g.variant) : ''}${g.designers.length ? ' · ' + esc(g.designers.join(', ')) : ''}</span></li>`).join('\n');
...
<body><main><h1>Big Head Club arcade</h1><p>${feed.count} games, most played first. A play is a run where the game reports runs, otherwise a visitor who clicked, started, or stayed ten seconds; crawlers never count. <a style="color:#f2812f" href="/api/games.json">feed</a></p><ol>${rows}</ol></main></body></html>`;
}
